...
class Screen
{
	...
	void SetPixel(int r, int c, double *col, double z, double shade){
		if(c >= 0 and r >= 0 and c < width and r < height){
			int index = 3 * (r * width + c);
			int zindex = (r * width + c);
			if(z > zbuffer[zindex]){
				buffer[index] = ceil_441(255 * std::min(1.0, shade * col[0]));
				buffer[index + 1] = ceil_441(255 * std::min(1.0, shade * col[1]));
				buffer[index + 2] = ceil_441(255 * std::min(1.0, shade * col[2]));
				zbuffer[zindex] = z;
			}
		}
	}
	...
	for(int r = ceil_441(rowMin); r <= floor_441(rowMax); r++){
		...
		if(leftEnd == x1 and rightEnd == x2){
			...
			leftZ = lerpZ(t.X[lowerY], rowMin, 
						  ...
						  leftEnd, r);
			leftShade = lerpZ(t.X[lowerY], rowMin, 
						  t.X[(lowerY+1)%3], rowMax, 
						  t.shading[lowerY], t.shading[(lowerY+1)%3], 
						  leftEnd, r);
			...
			rightZ = lerpZ(t.X[lowerY], rowMin, 
						   ...
						   rightEnd, r);
			rightShade = lerpZ(t.X[lowerY], rowMin, 
						   t.X[(lowerY+2)%3], rowMax, 
						   t.shading[lowerY], t.shading[(lowerY+2)%3], 
						   rightEnd, r);
		}
		else{
		...
		for(int c = ceil_441(leftEnd); c <= floor_441(rightEnd); c++){
			...
			pixelShade = lerpZ(leftEnd, r, rightEnd, r, leftShade, rightShade, c, r);
			s.SetPixel(r, c, pixelCol, pixelZ, pixelShade);
		}
	}
}
	...
	for(int r = ceil_441(rowMin); r <= floor_441(rowMax); r++){
		...
		if(leftEnd == x1){
			...
			leftZ = lerpZ(t.X[upperY], t.Y[upperY], 
						  ...
						  leftEnd, r);
			leftShade = lerpZ(t.X[upperY], t.Y[upperY], 
							t.X[(upperY+1)%3], t.Y[(upperY+1)%3], 
							t.shading[upperY], t.shading[(upperY+1)%3], 
							leftEnd, r);
			...
			rightZ = lerpZ(t.X[upperY], t.Y[upperY], 
						  ...
						  rightEnd, r);
			rightShade = lerpZ(t.X[upperY], t.Y[upperY], 
						  t.X[(upperY+2)%3], t.Y[(upperY+2)%3], 
						  t.shading[upperY], t.shading[(upperY+2)%3], 
						  rightEnd, r);
		}
		else{
...
double CalculatePhongShading(LightingParameters &lp, double *viewDirection, double *normal) {
	double diffuse = 0;
	double specular = 0;
	double normLightDir = 0;
	double normalizeLight[3] = {0,0,0};
	double normNormal = 0;
	double normalizeNormal[3] = {0,0,0};
	double dotLN = 0;
	double R[3] = {0,0,0};
	double normR = 0;
	double normalizeR[3] = {0,0,0};
	double normViewDir = 0;
	double normalizeView[3] = {0,0,0};
	double dotRV = 0;

	// Ambient has no differences! Woo!

	normLightDir = sqrt(lp.lightDir[0]*lp.lightDir[0] + 
						lp.lightDir[1]*lp.lightDir[1] + 
						lp.lightDir[2]*lp.lightDir[2]);
	normalizeLight[0] = lp.lightDir[0]/normLightDir; 
	normalizeLight[1] = lp.lightDir[1]/normLightDir; 
	normalizeLight[2] = lp.lightDir[2]/normLightDir;

	normNormal = sqrt(normal[0]*normal[0] + 
					  normal[1]*normal[1] + 
					  normal[2]*normal[2]);
	normalizeNormal[0] = normal[0]/normNormal; 
	normalizeNormal[1] = normal[1]/normNormal; 
	normalizeNormal[2] = normal[2]/normNormal;
	
	dotLN = normalizeLight[0]*normalizeNormal[0] + 
			normalizeLight[1]*normalizeNormal[1] + 
			normalizeLight[2]*normalizeNormal[2];

	diffuse = abs(dotLN);
	// Diffuse has 1 pixel difference (column = 458, row = 500)! Woo!

	
	R[0] = 2 * dotLN * normalizeNormal[0] - normalizeLight[0];
	R[1] = 2 * dotLN * normalizeNormal[1] - normalizeLight[1];
	R[2] = 2 * dotLN * normalizeNormal[2] - normalizeLight[2];

	normR = sqrt(R[0]*R[0] + 
				 R[1]*R[1] + 
				 R[2]*R[2]);

	normalizeR[0] = R[0]/normR;
	normalizeR[1] = R[1]/normR;
	normalizeR[2] = R[2]/normR;

	normViewDir = sqrt(viewDirection[0]*viewDirection[0] + 
					   viewDirection[1]*viewDirection[1] + 
					   viewDirection[2]*viewDirection[2]);

	normalizeView[0] = viewDirection[0]/normViewDir;
	normalizeView[1] = viewDirection[1]/normViewDir;
	normalizeView[2] = viewDirection[2]/normViewDir;

	dotRV = normalizeR[0]*normalizeView[0] + 
			normalizeR[1]*normalizeView[1] + 
			normalizeR[2]*normalizeView[2];

	specular = std::max(0.0, pow(dotRV, lp.alpha));
	// Specular had 1 pixel difference (column = 458, row = 500)! Woo!
...
void SplitTriangles(Screen s, Triangle t){
	...
	if(midY == maxY){
		...
		double splitShade = findEnd(t.shading[upperY], t.Y[upperY], 
							t.shading[lowerY], t.Y[lowerY], midY);;
		...
		downT.shading[upperY] = splitShade;
...
int main()
{
	...
	for(int k = 0; k < 1; k++){
		...
		for(int j = 0; j < triangles.size(); j++){
			for (int i = 0; i < 3; i++){
				viewDirection[0] = c.position[0] - triangles[j].X[i];
				viewDirection[1] = c.position[1] - triangles[j].Y[i];
				viewDirection[2] = c.position[2] - triangles[j].Z[i];
				triangles[j].shading[i] = CalculatePhongShading(lp, viewDirection, triangles[j].normals[i]);
			}
		}
