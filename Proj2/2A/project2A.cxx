 ...
 public:
   ...
   virtual void RenderPiece(vtkRenderer *ren, vtkActor *act)
   {
      ...
      for(int i = 0; i < triangles.size(); i++){
          for(int j = 0; j < 3; j++){
            glColor3ub(colors[3*(int)ceil(triangles[i].fieldValue[j]*255)], 
                       colors[3*(int)ceil(triangles[i].fieldValue[j]*255)+1], 
                       colors[3*(int)ceil(triangles[i].fieldValue[j]*255)+2]);
...
class vtk441MapperPart2 : public vtk441Mapper
{
 public:
   ...
   void SetUpTriangles(){
      triangles = GetTriangles();
      colors = GetColorMap();
      glGenTextures(1, &displayList);
      glBindTexture(GL_TEXTURE_1D, displayList);
      glTexParameterf(GL_TEXTURE_1D, GL_TEXTURE_WRAP_S, GL_REPEAT);
      glTexParameterf(GL_TEXTURE_1D, GL_TEXTURE_WRAP_T, GL_REPEAT);
      glTexParameterf(GL_TEXTURE_1D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
      glTexParameterf(GL_TEXTURE_1D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);

      glTexImage1D(GL_TEXTURE_1D, 0, GL_RGB, 256, 0, GL_RGB, GL_UNSIGNED_BYTE, colors);
